...
async function createPost() {
    console.log('=== POSZT LÉTREHOZÁS (ELSŐ BLOKK = CÍM) ===');
    
    // 1. Blokkok ellenőrzése
    const blocks = document.querySelectorAll('.content-block');
    if (blocks.length === 0) {
        alert('Legalább egy blokkot adj hozzá a poszthoz!\n\nAz első blokk lesz a címed.');
        return;
    }
    
    // 2. Az első blokk ellenőrzése (cím)
    const firstBlock = blocks[0];
    const firstBlockType = firstBlock.getAttribute('data-block-type');
    
    if (firstBlockType !== 'heading') {
        alert('Az első blokknak "Címsor" típusúnak kell lennie!\n\nEz lesz a posztod címe.');
        return;
    }
    
    // 3. Cím tartalom ellenőrzése
    const titleInput = firstBlock.querySelector('.block-heading-input');
    const title = titleInput ? titleInput.value.trim() : '';
    
    if (!title) {
        alert('Kérjük, adj meg egy címet az első blokkban!\n\nEz lesz a posztod címe.');
        if (titleInput) titleInput.focus();
        return;
    }
    
    if (title.length < 3) {
        alert('A cím túl rövid! Minimum 3 karaktert adj meg.');
        if (titleInput) titleInput.focus();
        return;
    }
    ...
    if (!token) {
        ...
        window.location.href = `/api/login/google?redirect=${encodeURIComponent(window.location.pathname)}`;
        return;
    }
    ...
    // 8. Kiemelt kép (ha van)
    let imageUrl = '/default-post-image.png';
    const featuredInput = document.getElementById('post-featured');
    if (featuredInput && featuredInput.files.length > 0) {
        console.log('Kiemelt kép feltöltése...');
        imageUrl = await uploadImage(featuredInput.files[0], token);
        if (!imageUrl) {
            alert('Hiba történt a kép feltöltése során!');
            return;
        }
        console.log('Kiemelt kép URL:', imageUrl);
    }
    
    // 9. Szerzők
    const authorsValue = document.getElementById('post-authors')?.value || '';
    const authors = formatAuthorsString(authorsValue);
    
    // 10. Kategória validálása
    ...
    // 11. API adatok összeállítása
    const postData = {
        title: title, // ⭐⭐⭐ AZ ELSŐ BLOKK CÍME ⭐⭐⭐
        category: category,
        number: nextNumber,
...
async function prepareBlockPostData() {
    ...
    // 6. Automatikus előnézet frissítése
...
async function processSingleBlock(block) {
    ...
    switch(blockType) {
        case 'paragraph':
            ...
            styles = getComputedStyles(block.querySelector('.block-content'));
            break;
        ...
        case 'image':
            ...
            if (img && img.src) {
                ...
                if (content.startsWith('data:image')) {
                    // Itt lehetne feltölteni, de most csak elmentjük
    ...
    // Stílusok gyűjtése
    const styleObj = getBlockStyles(block);
    if (Object.keys(styleObj).length > 0) {
        styles = { ...styles, ...styleObj };
    }
...
function initializeBlockEditor() {
    ...
    const blockEditorContainer = document.getElementById('block-editor-container');
        const existingBlocks = blockEditorContainer.querySelectorAll('.content-block');
    const noBlocksMsg = document.getElementById('no-blocks-message');
    if (existingBlocks.length > 0) {
        // Ha már vannak blokkok, rejtsük el az üzenetet
        if (noBlocksMsg) {
            noBlocksMsg.style.display = 'none';
        }
    } else {
        // Ha nincsenek blokkok, de az üzenet el van rejtve, jelenítsük meg
        if (noBlocksMsg && noBlocksMsg.style.display === 'none') {
