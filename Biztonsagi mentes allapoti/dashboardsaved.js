...
async function createPost() {
    console.log('=== POSZT LÉTREHOZÁS INDUL ===');
    ...
    console.log('Jogosultság OK, folytatás...');

    const titleInput = document.getElementById('post-title');
    const categoryInput = document.getElementById('post-categories'); // Ez a helyes ID
    ...
    if (!titleInput || !categoryInput || !contentInput) {
        console.error('Hiányzó űrlap elemek:', {
            title: !!titleInput,
            category: !!categoryInput,
            content: !!contentInput
        });
        alert('Hiba: Hiányzó űrlap elemek. Frissítsd az oldalt!');
        return;
    }
    
    const title = titleInput.value.trim();
    const category = categoryInput.value.trim(); // Itt a post-categories értékét olvassuk
    const minimal_desc = excerptInput ? excerptInput.value.trim() : '';
    const desc = contentInput.value.trim();
    const imageFile = featuredInput ? featuredInput.files[0] : null;
    const authorsValue = authorsInput ? authorsInput.value.trim() : '';
    const status = statusInput ? statusInput.value : 'draft';
    const editPid = document.getElementById('edit-pid')?.value;
    
    console.log('Alap adatok:', {
        title,
        category,
        minimal_desc,
    ...
    // Validáció
    if (!title) {
        alert('Kérjük, adja meg a poszt címét!');
        titleInput.focus();
        return;
    }
    
    // Ellenőrizzük, hogy van-e kategória
    if (!category || category.trim() === '') {
        alert('Kérjük, válasszon kategóriát!');
        // Kategória választó megnyitása
        const selectBtn = document.getElementById('select-category-btn');
        if (selectBtn) {
            selectBtn.click();
        }
        return;
    ...
    if (!desc) {
        alert('Kérjük, írja meg a poszt tartalmát!');
        contentInput.focus();
        return;
    }
    ...
    try {
        // Ellenőrizzük, hogy szerkesztés-e
        if (editPid) {
            console.log('Poszt szerkesztése:', editPid);
            await updateExistingPost(editPid, {
                title, category, minimal_desc, desc,
                authorsVaiue, status, token, imageFile
            });
            return;
        }
        
        console.log('1. Következő szám lekérése...');
        ...
        if (imageFile) {
            ...
            imageUrl = '/default-post-image.png';
            console.log('Alapértelmezett kép használata:', imageUrl);
        }
        
        const authors = formatAuthorsString(authorsValue);
        console.log('3. Szerzők formázva:', authors);
        ...
        console.log('4. Poszt adatok összeállítva:', postData);

        console.log('5. API hívás indítása /api/post/create...');
        ...
        if (!response.ok) {
            ...
            try {
                if (response.status === 401) {
                    ...
                    errorMessage = 'A poszt már létezik, vagy adatbázis hiba!';
                } else {
                    const errorText = await response.text();
                    console.error('Hiba részletei:', errorText);
                    errorMessage = `Hiba: ${response.statusText} - ${errorText}`;
                }
            } catch (parseError) {
                console.error('Parse hiba:', parseError);
                errorMessage = `Hálózati hiba: ${response.status}`;
            }
        ...
        try {
            ...
            console.log('6. Sikeres poszt létrehozás:', result);
            
            if (result.pid) {
                // Sikeres létrehozás után küldjük el ellenőrzésre
                await sendForReview(result.pid, token);
                
                // Frissítjük a következő számot
                try {
                ...
                // Sikeres üzenet és lista frissítés
                showSuccessMessage(result.pid);
                
                // ⭐⭐⭐ ÚJ: űrlap tartalmának törlése ⭐⭐⭐
                clearPostForm();
                
                // Vissza a cikkek listához
                setTimeout(() => {
                    document.querySelector('[data-target="articles"]').click();
                    loadUserArticles();
                }, 2000);
            } else {
                console.warn('Nincs PID a válaszban:', result);
                alert('Poszt létrehozva, de nem kaptunk PID-et vissza.');
            }
            
        } catch (jsonError) {
            console.error('JSON parse hiba a sikeres válasznál:', jsonError);
            alert('Poszt létrehozva, de nem sikerült értelmezni a választ.');
        }
        ...
        if (error.message.includes('Token') || error.message.includes('401')) {
            alert('A bejelentkezési munkameneted lejárt. Kérjük, jelentkezz be újra, majd próbáld újra a poszt létrehozását.');
            const currentPath = encodeURIComponent(window.location.pathname);
            window.location.href = `/api/login/google?redirect=${currentPath}`;
        } else {
